		...
		async buscarVendas() {
			...
			for (let i = 0; i < this.vendas.length; i++) {
				...
				for (let i = 0; i < produtos.length; i++) {
					let valorUnitario = parseFloat(produtos[i].valorUn);
					produtos[i].valorUn = "R$ " + valorUnitario.toFixed(2);
					let valorTotalProduto = parseFloat(produtos[i].valorTotProd);
					produtos[i].valorTotProd = "R$ " + valorTotalProduto.toFixed(2);
				}
